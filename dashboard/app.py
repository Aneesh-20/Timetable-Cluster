import sys, os
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(_file_))) if '_file_' in dir() else os.getcwd())
...
# ── CINEMATIC PRELOADER SPLASH LAYER ─────────────────────
if not st.session_state.splash_done:
    splash_window = st.empty()
    with splash_window.container():
        st.markdown(f"""
        <style>
