...
st.set_page_config(page_title="Slotra", layout="wide", page_icon="🗓️")
...
.logo-name{{
    font-size:26px;font-weight:800;color:{accent};
...
.sec-lbl{{
    font-size:11px;font-weight:700;color:{sub};
...
.splash-name{{
    font-size:42px;font-weight:900;color:#4FC3F7;
...
if st.session_state.page == "home":
    ...
    if generate:
        ...
        if not teachers_list:
            st.error("⚠️ Add at least one teacher.")
        elif not rooms_list:
            st.error("⚠️ Add at least one room.")
        elif not sections_list:
            st.error("⚠️ Add at least one section.")
        else:
    ...
    with tab4:
        ...
        with cr:
            ...
            fig_bar=px.bar(tl,x="Teacher",y="Total Periods",
                           ...
                           color="Total Periods",color_continuous_scale="Teal")
